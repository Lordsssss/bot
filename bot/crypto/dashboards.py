...
class TradingDashboard(BaseCryptoDashboard):
    ...
    async def _get_trading_embed(self) -> discord.Embed:
        ...
        # Current Holdings (similar to portfolio dashboard but more compact)
        if portfolio:
            holdings_text = ""
            for ticker, data in portfolio.items():
                if data['amount'] > 0:
                    ...
                    value = data['amount'] * current_price
                    coin_name = CRYPTO_COINS.get(ticker, {}).get('name', ticker)
                    holdings_text += f"**{coin_name}** ({ticker}): {data['amount']:,.2f} @ 🪙{current_price:.2f} = 🪙{value:,.0f}\n"
            
            if holdings_text:
                embed.add_field(name="💼 Current Holdings", value=holdings_text, inline=False)
            else:
                embed.add_field(name="💼 Current Holdings", value="No holdings found", inline=False)
        else:
            embed.add_field(name="💼 Current Holdings", value="No holdings found", inline=False)
